...
```bash
bash build.sh --extension ZileSarbatoare --zip 0.7.0 files scripts
```

Upload `dist/zile-sarbatoare-0.7.0.zip` in **Administration > Extensions**, or
install it from the EspoCRM root:

```bash
bin/command extension --file="/path/to/zile-sarbatoare-0.7.0.zip"
bin/command rebuild
