...
EspoCRM extension package for `Contact Customization`.

Package ID: `contact-customization`

Module code lives under:

```text
files/custom/Espo/Modules/ContactCustomization
```

Module ACL files can be placed under:

```text
files/custom/Espo/Modules/ContactCustomization/Resources/metadata/aclDefs
```
...
```bash
./build.sh --extension ./ContactCustomization --zip 1.0.0 files scripts
```
