...
Phase 15–23 runtime validation remains pending until an approved non-production EspoCRM 10.0.0 instance is provided. Install the rebuilt ZIP there, clear cache, run Administration > Rebuild, then exercise editor entry, save/reload/conflict protection, page settings, flow drag/drop, restricted rich-text paste and formatting, inline-variable placeholders, divider/spacer/page-break rendering and reordering, inherited styling/typography and Romanian diacritics, sample/empty rendering, validation issue focus, keyboard traversal, complex-node deletion confirmation, and literal XSS/CSS payload display as inert or rejected data. Never use `/opt/crm.cursurituv.ro` for these checks.
