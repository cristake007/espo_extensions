<?php

declare(strict_types=1);

if ($argc !== 2) {
    fwrite(STDERR, "Usage: php package-inventory.test.php /path/to/package.zip\n");
    exit(2);
}

$archive = new ZipArchive();

if ($archive->open($argv[1]) !== true) {
    throw new RuntimeException("Could not open package: {$argv[1]}");
}

foreach ([
    'files/client/custom/modules/document-builder/src/editor/geometry/page-geometry.js',
    'files/client/custom/modules/document-builder/src/editor/commands/update-document.js',
    'files/client/custom/modules/document-builder/src/editor/page-settings.js',
    'files/client/custom/modules/document-builder/src/views/editor/shell.js',
    'files/client/custom/modules/document-builder/res/templates/editor/shell.tpl',
    'files/client/custom/modules/document-builder/res/css/editor.css',
    'files/custom/Espo/Modules/DocumentBuilder/Tools/DocumentBuilder/Layout/LayoutValidator.php',
    'files/custom/Espo/Modules/DocumentBuilder/Resources/jsonSchema/document-builder-layout-v1.json',
] as $entry) {
    if ($archive->locateName($entry) === false) {
        throw new RuntimeException("Phase 18 package entry is missing: $entry");
    }
}

$archive->close();

echo "Phase 18 package inventory tests passed.\n";
