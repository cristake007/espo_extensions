# Custom Workflow Architecture - GeneratorPerioadeCursuri

Timestamp: 20260707-212958

## 1. Current Direction

The `GeneratorPerioadeCursuri` entity is the workflow record. Users enter the source file,
year, selected months, and holidays directly on a `GeneratorPerioadeCursuri` record, then
click a Generate button on that record.

The actual product workflow is:

1. Create or open a `GeneratorPerioadeCursuri` record.
...
The `GeneratorPerioadeCursuri` detail page is the user-facing generation screen.

Recommended UX:

- Keep the `GeneratorPerioadeCursuri` tab.
- Use standard `GeneratorPerioadeCursuri` create/edit/detail views for inputs that EspoCRM
  already handles: source file, year, months, holidays, users, and teams.
- Add a custom Generate button on the `GeneratorPerioadeCursuri` detail view.
...
- Do not keep `GeneratorPerioadeCursuriRow` as a separate metadata entity unless a later
...
files/custom/Espo/Modules/GeneratorPerioadeCursuri/Tools/GeneratorPerioadeCursuri/Api/PostGenerate.php
files/custom/Espo/Modules/GeneratorPerioadeCursuri/Tools/GeneratorPerioadeCursuri/Api/GetExport.php
files/custom/Espo/Modules/GeneratorPerioadeCursuri/Tools/GeneratorPerioadeCursuri/GenerationService.php
files/custom/Espo/Modules/GeneratorPerioadeCursuri/Tools/GeneratorPerioadeCursuri/CourseInputParser.php
files/custom/Espo/Modules/GeneratorPerioadeCursuri/Tools/GeneratorPerioadeCursuri/CourseScheduler.php
files/custom/Espo/Modules/GeneratorPerioadeCursuri/Tools/GeneratorPerioadeCursuri/XlsxExportService.php
...
[
    {
        "route": "/GeneratorPerioadeCursuri/generate",
        "method": "post",
        "actionClassName": "Espo\\Modules\\GeneratorPerioadeCursuri\\Tools\\GeneratorPerioadeCursuri\\Api\\PostGenerate"
    },
    {
        "route": "/GeneratorPerioadeCursuri/:id/exportXlsx",
        "method": "post",
        "actionClassName": "Espo\\Modules\\GeneratorPerioadeCursuri\\Tools\\GeneratorPerioadeCursuri\\Api\\PostExportXlsx"
    }
]
```

The Generate action should read the current `GeneratorPerioadeCursuri` record values,
...
```json
{
    "generatorPerioadeCursuriId": "existing-record-id"
}
```
...
```json
{
    "id": "generator-perioade-cursuri-id",
    "rowCount": 10,
    "rows": [
        {
...
- provide a link to the saved `GeneratorPerioadeCursuri` record;
...
1. Add `Generate` on the `GeneratorPerioadeCursuri` detail record view.
...
- Added packaged route `POST /GeneratorPerioadeCursuri/generate`.
- Added `PostGenerate` API action that confirms saved record values.
- Added a custom `GeneratorPerioadeCursuri` detail view with a Generate button.
...
- Create/update `GeneratorPerioadeCursuri`.
...
- keep `GeneratorPerioadeCursuri` as the workflow/input record;
