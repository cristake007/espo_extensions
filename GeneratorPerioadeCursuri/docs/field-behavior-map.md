# Field Behavior Map - Generator perioade cursuri

Timestamp: 20260707-195255

This document classifies current and planned `GeneratorPerioadeCursuri` fields by how users and later generation logic should treat them. It is based on direct inspection of:
...
- `extensions/GeneratorPerioadeCursuri/docs/conversion-map.md`
- current `entityDefs/GeneratorPerioadeCursuri.json`
- current `layouts/GeneratorPerioadeCursuri/*.json`
- current i18n files
...
| Generated schedule rows | `ScheduleGeneration.schedule`; generated JSON rows | Child entity `GeneratorPerioadeCursuriRow` | Manual only until service exists | Yes | Yes | Yes | Yes | Yes | Implemented in Phase 3E as metadata-only normalized rows, one record per course/month generated entry. No automatic generation exists yet. |
