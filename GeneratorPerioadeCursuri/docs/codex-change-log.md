...
- Renamed the extension, module, primary entity, matcher entity, PHP namespaces,
  client module, metadata, layouts, translations, and API routes from the
  `Planificari` naming family to `GeneratorPerioadeCursuri`.
- Changed the package slug to `generator-perioade-cursuri` and bumped the
  package version to `2.0.0`.
- Kept legacy menu and removed-row identifiers only in lifecycle compatibility
  cleanup so upgrades can remove stale navigation and metadata entries.
- Preserved older entries below as historical records of their original paths
  and identifiers.
