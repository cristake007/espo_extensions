# Conversion Map - Generator perioade cursuri
...
- `files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/module.json`: EspoCRM module registration.
- `files/custom/Espo/Modules/GeneratorPerioadeCursuri/Controllers/GeneratorPerioadeCursuri.php`: minimal standard record controller.
- `files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/scopes/GeneratorPerioadeCursuri.json`: existing generic scaffold entity scope.
- `files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/entityDefs/GeneratorPerioadeCursuri.json`: generic scaffold fields only: name, description, timestamps, users, and teams.
- `files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/clientDefs/GeneratorPerioadeCursuri.json`: standard record controller, only-my filter, calendar icon.
- `files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/aclDefs`, `entityAcl`, `recordDefs`: empty/basic scaffold ACL and dynamic logic metadata.
- `files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/i18n/en_US` and `ro_RO`: scaffold labels.
- `scripts/AfterInstall.php` and `scripts/BeforeUninstall.php`: add/remove `GeneratorPerioadeCursuri` from EspoCRM tab list.

The scaffold is a simple EspoCRM extension package, not the full `ext-template` npm scaffold. Runtime files will be installed under `custom/Espo/Modules/GeneratorPerioadeCursuri/...`. It can be built with the README ZIP command.
...
| --- | --- | --- | --- | --- | --- |
| `ScheduleGeneration` | Stores one generated schedule, source upload metadata, JSON schedule rows, original file bytes, owner, creation time, and expiration. | `GeneratorPerioadeCursuri` | `BasePlus` | Phase 2 minimal root, Phase 3 fields | Reuse the existing scaffold entity as the root generation record; do not add business fields in Phase 1. |
| `AppSetting` | Stores global/user workflow preferences by scope for generator, Word converter, and WordPress updater. | None in Phase 2 | Not an entity | Phase 10 review only | Do not create `AppSetting`, `GeneratorPerioadeCursuriSettings`, settings fields, custom PHP, custom JS, or Django-style settings storage. Later review EspoCRM user preferences, EspoCRM config, metadata defaults, or a small admin/settings mechanism. |
...
| Generated schedule rows inside `ScheduleGeneration.schedule` | JSON row | Generation | `generatorPerioadeCursuriRows` | One-to-many normalized child records | Implemented metadata-only in Phase 3E as `GeneratorPerioadeCursuriRow`; no parser/generator/export service yet. |
...
| --- | --- | --- | --- | --- | --- |
| `PeriodGeneratorView`, `generator_perioade.html` | Upload course list and generate periods. | Start with standard `GeneratorPerioadeCursuri` list/detail/edit layouts; custom action later only if needed. | Later likely yes | Later likely yes | Upload parsing and generation are workflow logic, not Phase 1. |
...
- Phase 2: Formalize the existing `GeneratorPerioadeCursuri` scaffold entity as the minimal root generation entity with only native/system fields and safe labels.
...
- Phase 5: Add the relationship between `GeneratorPerioadeCursuri` and generated schedule rows.
...
- Phase 2: Upload ZIP, install extension, rebuild, confirm the extension appears with the correct name and the `GeneratorPerioadeCursuri` tab/entity opens.
...
- The current scaffold already includes a generic `GeneratorPerioadeCursuri` entity. It should be treated as placeholder-only until Phase 2.
...
- Do not create `AppSetting`, `GeneratorPerioadeCursuriSettings`, settings fields, custom PHP for settings, custom JavaScript for settings, or Django-style settings storage in Phase 2.
