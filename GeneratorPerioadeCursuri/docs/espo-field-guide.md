# EspoCRM Field Guide - GeneratorPerioadeCursuri

This guide is intended to be copied into:

`extensions/GeneratorPerioadeCursuri/docs/espo-field-guide.md`

It is a local source of truth for Codex while converting the Django app `app_to_convert/planificator/` into the EspoCRM extension `extensions/GeneratorPerioadeCursuri/`.
...
`GeneratorPerioadeCursuri` and `GeneratorPerioadeCursuriRow` remain backend storage/history entities.
...
```text
extensions/GeneratorPerioadeCursuri/docs/custom-workflow-architecture.md
```
...
```text
extensions/GeneratorPerioadeCursuri/
├── manifest.json
├── files/
│   └── custom/Espo/Modules/GeneratorPerioadeCursuri/...
└── scripts/
```
...
```text
extensions/GeneratorPerioadeCursuri/.codex-backups/<timestamp>/
```

Update this file after every phase:

```text
extensions/GeneratorPerioadeCursuri/docs/codex-change-log.md
```
...
```text
extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/scopes/GeneratorPerioadeCursuri.json
extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/entityDefs/GeneratorPerioadeCursuri.json
extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/clientDefs/GeneratorPerioadeCursuri.json
extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/aclDefs/GeneratorPerioadeCursuri.json
extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/entityAcl/GeneratorPerioadeCursuri.json
extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/recordDefs/GeneratorPerioadeCursuri.json
extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/layouts/GeneratorPerioadeCursuri/
extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/i18n/en_US/GeneratorPerioadeCursuri.json
extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/i18n/ro_RO/GeneratorPerioadeCursuri.json
```
...
| EspoCRM type | Use for | Notes for GeneratorPerioadeCursuri |
| --- | --- | --- |
...
- `AppSetting`;
- `GeneratorPerioadeCursuriSettings`;
- settings fields;
- settings entity;
...
| `schedule` | Django JSON generated rows | Implemented as child row records metadata-only | No | Yes, generated result | No | Via `generatorPerioadeCursuriRows` relationship | Child rows are manually editable only until generation service exists | Child row list | Child row search | Child entity `GeneratorPerioadeCursuriRow` | Phase 3E | One `GeneratorPerioadeCursuriRow` per generated course/month entry. No parser, generator, export, or automatic row creation exists yet. |
...
| Phase 3E | Generated schedule rows | Implemented `GeneratorPerioadeCursuriRow` child entity and `generatorPerioadeCursuriRows` relationship metadata | No parser, algorithmic generation, export, custom PHP, or custom JavaScript. |
...
| Phase 4E | Persistence | Save `GeneratorPerioadeCursuri` and replace `GeneratorPerioadeCursuriRow` rows transactionally | Do not make users manually create rows. |
...
- `extensions/GeneratorPerioadeCursuri/docs/input file.csv`
- `extensions/GeneratorPerioadeCursuri/docs/generated file.xlsx`
...
- current `GeneratorPerioadeCursuri` metadata, layouts, and i18n
...
- Create `GeneratorPerioadeCursuriRow` as the child entity for generated schedule rows.
- Add parent link `GeneratorPerioadeCursuri.generatorPerioadeCursuriRows`.
- Add child link `GeneratorPerioadeCursuriRow.generatorPerioadeCursuri`.
...
Implemented `GeneratorPerioadeCursuriRow` fields:

| Field | Type | Source value | Notes |
| --- | --- | --- | --- |
| `name` | `varchar` | Row label | Required record name. Later service can derive it from title/month. |
| `generatorPerioadeCursuri` | `link` | Parent generation | Required link to `GeneratorPerioadeCursuri`. |
...
- `extensions/GeneratorPerioadeCursuri/docs/input file.csv`
- `extensions/GeneratorPerioadeCursuri/docs/generated file.xlsx`
- current `GeneratorPerioadeCursuri` entity metadata
- current `GeneratorPerioadeCursuriRow` entity metadata
- current extension controller `Controllers/GeneratorPerioadeCursuri.php`
...
1. User creates or edits a `GeneratorPerioadeCursuri` record.
...
7. Server parses the file and generates a complete set of `GeneratorPerioadeCursuriRow`
   records.
8. Detail view shows generated rows through the `generatorPerioadeCursuriRows` relationship.
9. Later, user downloads an XLSX export generated from `GeneratorPerioadeCursuriRow` records.
...
- `Controllers/GeneratorPerioadeCursuri.php`: add a record action endpoint later, only after
  confirming EspoCRM action method conventions locally.
- `Services/GeneratorPerioadeCursuriGenerationService.php`: orchestration service for one
  generation run.
- `Services/CourseInputParser.php`: CSV/XLSX parser.
- `Services/CourseScheduler.php`: PHP port of the scheduling algorithm.
- `Services/GeneratorPerioadeCursuriRowWriter.php`: deletes/replaces existing rows and
  persists generated `GeneratorPerioadeCursuriRow` records transactionally.
- `Services/GeneratorPerioadeCursuriExportService.php`: later XLSX export projection from
  stored rows.
...
- record-level action on an existing `GeneratorPerioadeCursuri` record;
...
Inputs read from `GeneratorPerioadeCursuri`:
...
For each generated item, create one `GeneratorPerioadeCursuriRow`:

| Generated value | `GeneratorPerioadeCursuriRow` field |
| --- | --- |
| derived label | `name` |
| parent record | `generatorPerioadeCursuri` |
| `Title` | `courseTitle` |
| `Permalink` | `permalink` |
...
- saved `GeneratorPerioadeCursuri` record;
- related `GeneratorPerioadeCursuriRow` records;
...
- Confirm EspoCRM custom action method conventions before editing
  `Controllers/GeneratorPerioadeCursuri.php`.
...
- `docker-compose.yml`
- `custom/Espo/Modules/GeneratorPerioadeCursuri/Controllers/GeneratorPerioadeCursuri.php`
- `custom.container-backup/Espo/Custom/Controllers/CCursuri.php`
- `custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/entityDefs/GeneratorPerioadeCursuri.json`
- `extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/entityDefs/GeneratorPerioadeCursuri.json`
...
```php
class GeneratorPerioadeCursuri extends \Espo\Core\Templates\Controllers\BasePlus
{
}
```
...
- the current `GeneratorPerioadeCursuri` controller is a standard record controller only.
...
- do not edit `Controllers/GeneratorPerioadeCursuri.php` yet;
...
GeneratorPerioadeCursuri extension package:

- `extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/entityDefs/GeneratorPerioadeCursuri.json`
- `extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/layouts/GeneratorPerioadeCursuri/detail.json`
- `extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/layouts/GeneratorPerioadeCursuri/edit.json`
- `extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/layouts/GeneratorPerioadeCursuri/list.json`
- `extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/layouts/GeneratorPerioadeCursuri/search.json`
- `extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/i18n/en_US/GeneratorPerioadeCursuri.json`
- `extensions/GeneratorPerioadeCursuri/files/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/i18n/ro_RO/GeneratorPerioadeCursuri.json`

Local metadata examples:

- `/opt/crm.cursurituv.ro/custom/Espo/Modules/GeneratorPerioadeCursuri/Resources/metadata/entityDefs/GeneratorPerioadeCursuri.json`
...
Current GeneratorPerioadeCursuri layouts use the standard field reference shape:
...
4. Read current `entityDefs/GeneratorPerioadeCursuri.json`.
...
## ZIP
extensions/generator-perioade-cursuri-x.y.z.zip
...
Use extensions/GeneratorPerioadeCursuri/docs/espo-field-guide.md as the source of truth.
